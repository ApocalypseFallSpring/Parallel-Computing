...
#define F_SIMD_16(x, y, z) { \
  vorrq_u32(vandq_u32((x).val[0], (y).val[0]), vandq_u32(vmvnq_u32((x).val[0]), (z).val[0])), \
  vorrq_u32(vandq_u32((x).val[1], (y).val[1]), vandq_u32(vmvnq_u32((x).val[1]), (z).val[1])), \
  vorrq_u32(vandq_u32((x).val[2], (y).val[2]), vandq_u32(vmvnq_u32((x).val[2]), (z).val[2])), \
  vorrq_u32(vandq_u32((x).val[3], (y).val[3]), vandq_u32(vmvnq_u32((x).val[3]), (z).val[3]))  \
}

#define G_SIMD_16(x, y, z) { \
    vorrq_u32(vandq_u32((x).val[0], (z).val[0]), vandq_u32((y).val[0], vmvnq_u32((z).val[0]))), \
    vorrq_u32(vandq_u32((x).val[1], (z).val[1]), vandq_u32((y).val[1], vmvnq_u32((z).val[1]))), \
    vorrq_u32(vandq_u32((x).val[2], (z).val[2]), vandq_u32((y).val[2], vmvnq_u32((z).val[2]))), \
    vorrq_u32(vandq_u32((x).val[3], (z).val[3]), vandq_u32((y).val[3], vmvnq_u32((z).val[3]))) \
}

#define H_SIMD_16(x, y, z) { \
    veorq_u32(veorq_u32((x).val[0], (y).val[0]), (z).val[0]), \
    veorq_u32(veorq_u32((x).val[1], (y).val[1]), (z).val[1]), \
    veorq_u32(veorq_u32((x).val[2], (y).val[2]), (z).val[2]), \
    veorq_u32(veorq_u32((x).val[3], (y).val[3]), (z).val[3]) \
}

#define I_SIMD_16(x, y, z) { \
    veorq_u32((y).val[0], vorrq_u32((x).val[0], vmvnq_u32((z).val[0]))), \
    veorq_u32((y).val[1], vorrq_u32((x).val[1], vmvnq_u32((z).val[1]))), \
    veorq_u32((y).val[2], vorrq_u32((x).val[2], vmvnq_u32((z).val[2]))), \
    veorq_u32((y).val[3], vorrq_u32((x).val[3], vmvnq_u32((z).val[3]))) \
}

#define FF_SIMD_16(a, b, c, d, x, s, ac) do { \
    uint32x4x4_t _f_temp = F_SIMD_16((b), (c), (d)); \
    for(int _i=0; _i<4; ++_i) { \
        uint32x4_t _temp = vaddq_u32(vaddq_u32(_f_temp.val[_i], (x).val[_i]), vdupq_n_u32(ac)); \
        (a).val[_i] = vaddq_u32((a).val[_i], _temp); \
        (a).val[_i] = ROTATELEFT_SIMD((a).val[_i], (s)); \
        (a).val[_i] = vaddq_u32((a).val[_i], (b).val[_i]); \
    } \
} while(0)


#define GG_SIMD_16(a, b, c, d, x, s, ac) do { \
    uint32x4x4_t _g_temp = G_SIMD_16((b), (c), (d)); \
    for (int _i = 0; _i < 4; ++_i) { \
        uint32x4_t _temp = vaddq_u32(vaddq_u32(_g_temp.val[_i], (x).val[_i]), vdupq_n_u32(ac)); \
        (a).val[_i] = vaddq_u32((a).val[_i], _temp); \
        (a).val[_i] = ROTATELEFT_SIMD((a).val[_i], (s)); \
        (a).val[_i] = vaddq_u32((a).val[_i], (b).val[_i]); \
    } \
...
#define HH_SIMD_16(a, b, c, d, x, s, ac) do { \
    uint32x4x4_t _h_temp = H_SIMD_16((b), (c), (d)); \
    for (int _i = 0; _i < 4; ++_i) { \
        uint32x4_t _temp = vaddq_u32(vaddq_u32(_h_temp.val[_i], (x).val[_i]), vdupq_n_u32(ac)); \
        (a).val[_i] = vaddq_u32((a).val[_i], _temp); \
        (a).val[_i] = ROTATELEFT_SIMD((a).val[_i], (s)); \
        (a).val[_i] = vaddq_u32((a).val[_i], (b).val[_i]); \
    } \
...
#define II_SIMD_16(a, b, c, d, x, s, ac) do { \
    uint32x4x4_t _i_temp = I_SIMD_16((b), (c), (d)); \
    for (int _i = 0; _i < 4; ++_i) { \
        uint32x4_t _temp = vaddq_u32(vaddq_u32(_i_temp.val[_i], (x).val[_i]), vdupq_n_u32(ac)); \
        (a).val[_i] = vaddq_u32((a).val[_i], _temp); \
        (a).val[_i] = ROTATELEFT_SIMD((a).val[_i], (s)); \
        (a).val[_i] = vaddq_u32((a).val[_i], (b).val[_i]); \
    } \
