...
public:
    ...
    void GenerateWithGPU(const string& prefix, segment* a, int n);
    
    //void GenerateBatchWithGPU(const vector<PT>& batch_pts);

    void PopNextBatch(int batch_size);
    void GenerateBatchWithGPU(const vector<PT>& batch);

    void ProcessBatchAsync(vector<PT>& batch, cudaStream_t stream); 

    int total_guesses = 0;
    vector<string> guesses;

    // GPU内存跟踪
    vector<GPUMemory> gpu_memories;
    
    // 新增：异步生成方法
    void GenerateWithGPUAsync(const string& prefix, segment* a, int n, cudaStream_t stream, GPUMemory& gpu_mem);
    void GenerateBatchWithGPUAsync(const vector<PT>& batch, cudaStream_t stream);
    void FinalizeGPUResults(cudaStream_t stream);
