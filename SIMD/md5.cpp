...
void MD5HashSIMD8(const vector<string>& inputs, vector<bit32*>& states) {
    ...
    // 遍历批量中的每个块
    for(size_t i = 0; i < n_block; ++i) {
        uint32x4x2_t x[16];  // 存储 8 个输入的 16 个 32 位字

        // 加载 8 个输入的块数据到 x[0..15]
        for (int j = 0; j < 16; ++j) {
			const uint32_t* temp_low[4] = {
				reinterpret_cast<const uint32_t*>(&paddedMessages[0][i * 64]),
				reinterpret_cast<const uint32_t*>(&paddedMessages[1][i * 64]),
				reinterpret_cast<const uint32_t*>(&paddedMessages[2][i * 64]),
				reinterpret_cast<const uint32_t*>(&paddedMessages[3][i * 64])
			};
			const uint32_t* temp_high[4] = {
				reinterpret_cast<const uint32_t*>(&paddedMessages[4][i * 64]),
				reinterpret_cast<const uint32_t*>(&paddedMessages[5][i * 64]),
				reinterpret_cast<const uint32_t*>(&paddedMessages[6][i * 64]),
				reinterpret_cast<const uint32_t*>(&paddedMessages[7][i * 64])
			};
			uint32x4_t temp_vec_low = {
				temp_low[0][j], temp_low[1][j], temp_low[2][j], temp_low[3][j]
			};
			uint32x4_t temp_vec_high = {
				temp_high[0][j], temp_high[1][j], temp_high[2][j], temp_high[3][j]
			};

			x[j].val[0] = temp_vec_low;
			x[j].val[1] = temp_vec_high;
        }
		...
		// print_uint32x4x2_t("a", a);
		// print_uint32x4x2_t("b", b);
		// print_uint32x4x2_t("c", c);
		// print_uint32x4x2_t("d", d);
